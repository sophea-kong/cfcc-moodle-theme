#!/usr/bin/env python3
"""
Convert Python exercise definitions (JSON) to Moodle XML format
for import into the CodeRunner question bank.

Usage:
    ./scripts/python-to-moodle.py exercises/python-examples.json
    ./scripts/python-to-moodle.py exercises/python-examples.json -o python-bank.xml
"""
...
def main():
    parser = argparse.ArgumentParser(description="Python exercises → Moodle XML (CodeRunner)")
    parser.add_argument("input", type=str, help="JSON exercise file")
    parser.add_argument("-o", "--output", type=str, default="moodle-python-questions.xml",
                        help="Output path (default: moodle-python-questions.xml)")
    ...
    print(f"✓ {len(exercises)} Python question(s) → {output_path}")
